...
C {devices/vsource.sym} -200 70 0 0 {name=V4 value="\{0.8-indiff/2\}"}
C {devices/vsource.sym} -300 70 0 0 {name=V5 value="\{0.8+indiff/2\}"}
...
* Voltage source value is set as "\{0.8+indiff/2\}"
.param indiff = 300m
...
.control 
  ...
  while run < nruns
    ...
    * save outputs to a file, changing the filename every iteration
    write loop_test_\{$&run\}.raw
    wrdata loop_test_\{$&run\}.txt clk ina inb
