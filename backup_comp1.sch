...
while run < nruns
  echo top of loop
  echo loop var is \{$&run\}

* note that "reset" is required for alterparam to work
  set run_var = "$&run"
  echo run_var is \{$run_var\}

  let indiff_value  = indiffs[run]
  echo indiff_value is \{$&indiff_value\}
  ...
  tran 0.5n 10u
  *write $rawfile testout\{$indiff_i\}.allv


  write $rawfile
  write testout\{$&run\}.raw
